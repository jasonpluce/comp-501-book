...
• Learning Objectives  
o Define "exception" and describe the difference between syntax and runtime 
errors.  
o Predict and interpret traceback messages in Python.  
o Use try/except/else/finally to catch and respond to errors.  
o Raise exceptions with raise ValueError() and handle multiple exception types.  
o Debug basic file handling and data conversion errors safely.  
• Lesson Outline  
o Warm-up (5 min)  
...
o Lab 1: Write safe_divide(a, b) that returns a/b or None and prints a helpful 
message. Test with pairs (4,2), (5,0), ("x", 2).  
o Lab 2: Open numbers.txt. Each line has one value. Sum valid ints and skip 
invalid lines using try/except. Try using raise ValueError if the file is empty.  
• Reflection & Assessment (10 min)  
o What is the difference between detecting and handling errors?  
o When is it better to prevent than to catch?  
o Write a try/except that opens a file and prints 'no file found' if missing, 
otherwise prints its first line.  
• Optional HW  
o Modify Lab 2 to log errors to error_log.txt instead of printing them.  
o Research assertand write 2 assertions that prevent invalid inputs.
